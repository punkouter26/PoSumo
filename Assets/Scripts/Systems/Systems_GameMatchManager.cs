...
namespace PoSumo
{
    ...
    public sealed partial class Systems_GameMatchManager : MonoBehaviour
    {
        ...
        // Fallback only: AdoptCharacterIdentity overwrites both from the character
        // sheets the moment a bout is set up. Kept in step with behaviorName.
        public string nameB = "GRANDMA";
        ...
        private void BuildTopBar()
        {
            ...
            if (Systems_TournamentState.Active)
            {
                ...
                // Compact badge (checklist #2): "QUARTERFINAL 1/7" not
        ...
        /// Where Systems_FightHud's stats table lands at match end — results
        /// merged with telemetry (zero-scroll checklist #1). Built here, filled
        /// by the fight HUD's OnMatchEnded handler reparenting its detail card.
        public VisualElement ResultStatsHost { get; private set; }

        /// Promotion/demotion badge in the result card's header. Filled (and the
        /// pending rank change CONSUMED) in RevealResultCard so the announcement
        /// has exactly one surface — idea #8, ceremony merged into the result card.
        ...
        private void BuildResultCard()
        {
            ...
            // Padding diet (checklist #2): SPACE_5 -> SPACE_4. The card now also
            // carries the stats table and possibly a rank badge; its frame gives
            // back what the merged content costs. Buttons stay full size.
            _resultCard.Pad(Systems_UiKit.SPACE_4, Systems_UiKit.SPACE_4);
            ...
            // Rank badge (idea #8): the banzuke promotion moment, folded into the
            // one card the player is already dismissing — one surface, one
            // CONTINUE, instead of a result card followed by a ceremony modal.
            // Hidden until RevealResultCard finds a pending rank change.
            _rankBadge = Systems_UiKit.Text("", Systems_UiKit.FONT_SMALL,
                                            Systems_UiKit.Gold, true);
            _rankBadge.style.unityTextAlign = TextAnchor.MiddleCenter;
            _rankBadge.style.whiteSpace = WhiteSpace.Normal;
            _rankBadge.style.marginTop = Systems_UiKit.SPACE_2;
            _rankBadge.style.display = DisplayStyle.None;
            _rankBadge.NoPick();
            _resultCard.Add(_rankBadge);

            // The stats table joins the card at match end (see ResultStatsHost).
            ResultStatsHost = new VisualElement();
            _resultCard.Add(ResultStatsHost);
            ...
            _rematchButton.style.marginTop = Systems_UiKit.SPACE_4;
            ...
            _continueButton.style.marginTop = Systems_UiKit.SPACE_4;
        ...
        private void RevealResultCard()
        {
            ...
            // The one announcement of a rank change (idea #8): consume the pending
            // promotion/demotion HERE, inside the card the player is already
            // reading, so the bracket's ceremony and news line do not fire a
            // second time for the same rung. When nothing is pending this just
            // hides the badge for the next match.
            FillRankBadge();
        ...
        /// Fills (or clears) the result card's rank badge, consuming the pending
        /// rank change if there is one. Consume-once is the recorder's own rule —
        /// this is simply the FIRST reader now, and the bracket's ceremony stays
        /// as the fallback for a change that was never shown here (a reveal that
        /// never ran, or recordCareerStats off means no pending change at all).
        private void FillRankBadge()
        {
            if (_rankBadge == null)
            {
                return;
            }
            if (Systems_CareerRecorder.TryTakeRankChange(
                    out Systems_CareerRecorder.RankChange change))
            {
                string who = change.Fighter.ToUpperInvariant();
                string text = $"{who} {(change.Promoted ? "PROMOTED" : "DEMOTED")} TO {change.ToRank}";
                if (change.Promoted && !string.IsNullOrEmpty(change.Note))
                {
                    text += "  ·  " + change.Note;
                }
                _rankBadge.text = text;
                _rankBadge.style.color = change.Promoted
                    ? Systems_UiKit.Gold : Systems_UiKit.Bad;
                _rankBadge.style.display = DisplayStyle.Flex;
                _rankBadge.FadeIn(Systems_UiKit.MOTION_FAST);
                Systems_Log.Info($"[BANZUKE] {text}");
            }
            else
            {
                _rankBadge.style.display = DisplayStyle.None;
